...
If ContainsChinese(baseMessage) Then
    MsgBox "消息模板包含中文，请使用纯英文模板！", vbCritical, "错误"
    WScript.Quit
End If
...
If sendMethod <> "1" And sendMethod <> "2" Then
    ...
    WScript.Quit
End If
result = MsgBox("设置摘要：" & vbCrLf & "消息模板: " & baseMessage & vbCrLf & "发送条数: " & iterations & vbCrLf & "发送速度: " & speed & "条/秒" & vbCrLf & "发送方式: " & IIf(sendMethod = "1", "Enter", "Ctrl+Enter") & vbCrLf & vbCrLf & "是否开始执行？", vbYesNo + vbInformation, "确认设置")
If result = vbNo Then
    WScript.Quit
End If
